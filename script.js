...
function drawStars(t=0){ctx.clearRect(0,0,innerWidth,innerHeight);stars.forEach(s=>{const a=s.a*(.65+.35*Math.sin(t*.001+s.x));ctx.beginPath();ctx.fillStyle='rgba(210,202,235,'+a+')';ctx.arc(s.x,(s.y+t*s.s)%innerHeight,s.r,0,Math.PI*2);ctx.fill()});if(!reduced)requestAnimationFrame(drawStars)}
makeStars(); addEventListener('resize',makeStars); if(!reduced) requestAnimationFrame(drawStars); else drawStars(0);

if(!reduced && motionAvailable){
  const lenis=typeof Lenis !== 'undefined' ? new Lenis({duration:1.15,smoothWheel:true,wheelMultiplier:.9}) : null;
  smoothScroll=lenis;
  if(lenis){
...
tabs.forEach(tab=>tab.addEventListener('click',()=>{tabs.forEach(t=>{t.classList.remove('active');t.setAttribute('aria-selected','false')});panels.forEach(p=>{p.classList.remove('active');p.style.opacity=0});tab.classList.add('active');tab.setAttribute('aria-selected','true');const p=document.querySelector('#'+tab.dataset.tab);p.classList.add('active');if(reduced||!motionAvailable)p.style.opacity=1;else gsap.fromTo(p,{opacity:0,y:25},{opacity:1,y:0,duration:.6,ease:'power3.out'});if(motionAvailable)ScrollTrigger.refresh()}));

const profileModal=document.querySelector('#profile-modal');
const profileClose=profileModal?.querySelector('.modal-close');
const modalImage=profileModal?.querySelector('img');
...
function openDocument(trigger,src,title){
  modalOpener=trigger;
  modalImage.src=src;
  modalImage.alt=title+' 전체 이미지';
  modalTitle.textContent=title;
  profileModal.hidden=false;
  profileModal.querySelector('.profile-modal-image').scrollTop=0;
  document.body.classList.add('modal-open');
  backgroundRegions.forEach(el=>el.inert=true);
  smoothScroll?.stop();
  profileClose.focus();
  if(!reduced && motionAvailable)gsap.fromTo('.profile-modal-shell',{opacity:0,scale:.97},{opacity:1,scale:1,duration:.25});
}
document.querySelector('.portrait')?.addEventListener('click',event=>openDocument(event.currentTarget,'assets/profile-202609.webp','이은규 프로필 이력서'));
document.querySelectorAll('[data-document]').forEach(link=>link.addEventListener('click',event=>{
  event.preventDefault();
  openDocument(link,link.dataset.document,link.dataset.title);
}));
